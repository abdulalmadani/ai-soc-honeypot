...
### Gemini rationalized a real breach, but the rule caught it
...
*First real attack data, with the top attacker's failed-login volume climbing hour over hour against the honeypot.*
...
The agent runs on demand, not as an always-on service: each `python agent/agent.py` is a single manual pass over the live workspace.
